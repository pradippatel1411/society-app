...
export default function SuperAdminLogin() {
  ...
  // If already logged in as the right super admin, send to dashboard
  useEffect(() => {
    if (
      isAuthenticated &&
...
useEffect(() => {
  ...
  async function handleSendOtp(e?: React.FormEvent) {
    ...
    try {
      const res = await api<{
        ...
        method: "POST",
        body: { mobile, scope: "super_admin", scopeRef: slug },
      })
      setStep("otp")
        ...
        <Card className="max-w-md w-full">
          <CardBody className="text-center py-12">
            ...
            <p className="text-sm text-ink-muted">
              The link <span className="tabular">/{slug}</span> isn't valid.
  ...
  const accent = branding.brandColor || "#1A1F2E"
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-cream relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md animate-slide-up">
        <div className="text-center mb-8">
          ...
          <h1 className="font-serif text-3xl text-ink">{branding.name}</h1>
          <p className="text-ink-muted text-sm mt-2">
            {step === "mobile"
              ? "Sign in to manage your societies"
            ...
            {locState.loggedOut && locState.loginUrl && (
              <div className="mb-5 rounded-lg bg-emerald-50 border border-emerald-200 px-4 py-3">
                ...
                <p className="text-xs text-emerald-700 mb-2">
                  Bookmark your society login link for next time:
                </p>
